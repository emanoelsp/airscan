...
export default function DiagnosticoIAPage() {
  ...
  useEffect(() => {
    getDocs(collection(db, "airscan_networks")).then((snap) => {
      setNetworks(snap.docs.map((d) => ({ id: d.id, ...d.data() } as Network)));
    });
    getDocs(collection(db, "airscan_assets")).then((snap) => {
  ...
  useEffect(() => {
    ...
    getDocs(query(collection(db, "airscan_diagnostico_ia"), ...constraints, limit(500)))
      .then((snap) => {
        let rows = snap.docs.map((d) => ({ id: d.id, ...d.data() } as DiagnosticoRow));
        if (networkId !== "all") {
          const assetIdsInNetwork = new Set(assets.filter((a) => a.networkId === networkId).map((a) => a.id));
          rows = rows.filter((r) => r.assetId && assetIdsInNetwork.has(r.assetId));
        }
        rows.sort((a, b) => {
  ...
  }, [period, networkId, assetId, severidade, assets]);

  const totalEventos = data.length;
  const custoTotal = data.reduce((s, r) => s + (Number(r.custo_estimado) || 0), 0);
  const topLpm = data.length
    ? data.reduce((a, b) => ((b.lpm ?? 0) > (a.lpm ?? 0) ? b : a), data[0])
    : null;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="mt-2 text-lg text-slate-400">
            Falhas, vazamentos e custos — dados da collection airscan_diagnostico_ia
          </p>
        </div>
